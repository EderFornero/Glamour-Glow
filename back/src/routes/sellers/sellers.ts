...
router.get("/", passport.authenticate('jwt',{session: false}),getSellersController)
router.get("/:id",passport.authenticate('jwt',{session: false}),schemaValidation(readAndDeleteSellerSchema), getSellersByIdController)
router.post("/",passport.authenticate('jwt',{session: false}), schemaValidation(createSellerSchema), postSellersController)
